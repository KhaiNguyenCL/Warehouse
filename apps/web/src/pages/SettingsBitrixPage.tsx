// Cấu hình mapping field Quotation ↔ field Bitrix (CLAUDE.md mục 13): "Bitrix Field Mapping:
// Cấu hình trong Settings, admin tự map không cần dev." PUT /bitrix/mappings luôn REPLACE
// TOÀN BỘ — dùng 1 Form.List + 1 nút lưu duy nhất, giống TemplateMappingsPanel.
...
// Khớp ALLOWED_QUOTATION_FIELDS ở bitrix.service.ts — chỉ field text tự do sửa ở Draft mới
// cho map từ Bitrix, không map vào field tính toán/trạng thái/company_id/contact_id.
...
export default function SettingsBitrixPage() {
  ...
  return (
    <div>
      <PageHeader title="Bitrix Field Mapping" />
      <Typography.Text type="secondary">
        Map field nào của Báo giá sẽ được điền tự động khi đồng bộ từ 1 Bitrix Deal. Bitrix chỉ ĐỌC — không ghi
        ngược lại Bitrix.
      </Typography.Text>

      <Form form={form} onFinish={(v) => saveMutation.mutate(v.mappings ?? [])} style={{ marginTop: 16 }}>
